...
const ImageDisplayPage = () => {
  ...
  const saveUrlsFromS3 = async () => {
    ...
    // Get the sorted pipe keys
    const pipeKeys = Object.keys(data.images);

    console.log("PIPE KEYS:", pipeKeys);

    for (const pipeKey of pipeKeys) {
      console.log("CURRENT PIPE KEY IN PROGRESS", pipeKey);
        const pipeArray = data.images[pipeKey];

        if (pipeArray.length > 0) {
        const usableUrls = await Promise.all(
          pipeArray.map(async (pipe, index) => {
            const url = pipe.url;
            ...
            return {
              index,
              url: URL.createObjectURL(blobThing)
            };
          }));
        //allUsableUrls[pipeKey] = usableUrls;
        allUsableUrls[pipeKey] = usableUrls.sort((a, b) => a.index - b.index).map(item => item.url);
        console.log("allUsableUrls Up To Now:", allUsableUrls);
        }
      }
