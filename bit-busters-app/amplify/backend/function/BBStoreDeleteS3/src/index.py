    ...
    try:
        ...
        if path.startswith('/s3bucket/images'):
            if http_method == 'POST':
                ...
                # Iterate through the pipes and their associated image URLs
                for pipe, image_urls in body.items():
                    ...
                    for key, image_url in image_urls.items():
                        try:
                            ...
                            #     response_data[pipe][key] = 'Invalid URL'
                            #     continue
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    return {'statusCode': 400, 'body': json.dumps({'message': 'Invalid image data URL format'})}

                                # Decode the Base64 content
                                try:
                                    decoded_content = base64.b64decode(encoded)
                                except base64.binascii.Error as b64_error:
                                    return {'statusCode': 400, 'body': json.dumps({'message': f'Error decoding Base64 data: {b64_error}'})}
                                
                                # Process the MIME type and file extension as in your original code
                                mime_type_match = re.search(r'data:(image/[^;]+);', header)
                                if mime_type_match:
                                    mime_type = mime_type_match.group(1)
                                    file_extension = mime_type.split('/')[-1]
                                else:
                                    return {'statusCode': 400, 'body': json.dumps({'message': 'Invalid MIME type in data URL'})}
                ...
                if not image_key:
                    return {'statusCode': 400, 'body': json.dumps({'message': 'Missing image_key'})}
                ...
                try:
                    ...
                    return {'statusCode': 500, 'body': json.dumps({'message': 'Error deleting image'})}
            ...
            if http_method == 'POST':
                ...
                if not data_url:
                    return {'statusCode': 400, 'body': json.dumps({'message': 'Missing signature data URL'})}
                ...
                try:
                    ...
                    try:
                        ...
                        return {'statusCode': 400, 'body': json.dumps({'message': 'Invalid signature data URL format'})}
                ...
                if not old_signature_key or not new_signature_data_url:
                    return {'statusCode': 400, 'body': json.dumps({'message': 'Missing old_signature_key or new_signature_data_url'})}
                ...
                if not signature_key:
                    return {'statusCode': 400, 'body': json.dumps({'message': 'Missing signature_key'})}
                ...
                try:
                    ...
                    print(f"Error deleting image: {e}")
                    return {'statusCode': 500, 'body': json.dumps({'message': 'Error deleting signature'})}
            ...
            if event['httpMethod'] == 'POST': 
                ...
                if not pdf_data_url:
                    return {'statusCode': 400, 'body': json.dumps({'message': 'Missing PDF data URL'})}
                try:
                    # Split the data URL into header and encoded content
                    try:
                        header, encoded = pdf_data_url.split(",", 1)
                    except ValueError:
                        return {'statusCode': 400, 'body': json.dumps({'message': 'Invalid pdf data URL format'})}
